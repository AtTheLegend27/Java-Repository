...
public class DLListTest {
    ...
    public void testDLListreverse(){
        DLListFactory<Integer> factory = new DLListFactory<>() ;
        DLList<Integer> test1 = factory.of ( 1, 2,3,4,5);
        DLList<Integer> expectedReversed = factory. of (  5, 4, 3, 2, 1);

        test1.reverse();
        assertThat(test1.size()).isEqualTo(5);
        assertThat(test1).isEqualTo(expectedReversed);


    public void testDLListreverse2(){
        DLListFactory<Integer> factory = new DLListFactory<> () ;
        DLList<Integer> test1 - factory.of ( ..values: 1) ;
        DLList<Integer> expectedReversed = factory.of ( ..values: 1);
        test1.reverse ();
        assertThat (test1.size()) .isEqualTo ( expected: 1) ; assertThat (test1) .isEqualTo (expectedReversed);
    }


        public void testDLListreverse3 () {
            DLListFactory<Integer> factory = new DLListFactory<> () ;
            DLList<Integer> test1 = factory.of () ;
            test1.reverseO);
            assertThat (test1.size()).isEqualTo (expected: 0) ;
}
