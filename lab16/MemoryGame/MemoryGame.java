...
public class MemoryGame {
    ...
    public String generateRandomString(int n) {
        ...
        int charTracker = n;
        while (charTracker > 0){
            int charIndex = RandomUtils.uniform(rand, n);
            char toAdd = CHARACTERS[charIndex];
            finalString.append(toAdd);
            charTracker--;
        }
    ...
    public void flashSequence(String letters) {
        ...
        for (int i = 0; i <= letters.length(); i++) {
            String drawThis = String.valueOf(characters[i]);
            drawFrame(drawThis);
        }
    }
    ...
    public void startGame() {
        ...
        while (!gameOver) {
            ...
            String tester = generateRandomString(round);
            flashSequence(tester);
            String input = solicitNCharsInput(round);
            if (tester.equals(input)) {
                break;
            }
        }
